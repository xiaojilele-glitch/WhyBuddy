/**
 * SlideRuleDev — V5 工程驾驶舱（/sliderule/dev）
 *
 * 严格遵循 V5 闭环（修复版）：
 ...
 *
 * 页面绝不直连 orchestrateReasoningTurn；全部经 SlideRuleRuntime.intakeMessage + markAwaiting。
 *
...
export default function SlideRuleDev() {
  ...
  // Subsequent loads/saves go through the (now async) store so that HttpSlideRuleSessionStore
  ...
  const sendMessage = async () => {
    ...
    workingState = await SlideRuleRuntime.saveSessionState(
      SlideRuleRuntime.markAwaiting(workingState, turnId)
    );
  ...
  async function runReentryTurn(intervention: UserIntervention, turnId: string, forceFail = false) {
    ...
    working = await SlideRuleRuntime.saveSessionState(
      SlideRuleRuntime.markAwaiting(working, turnId)
    );
