...
export interface ArtifactFragment {
  label: "结论" | "风险" | "反驳" | "建议" | "证据" | "片段"; // 保持中文用于现有页面 filter + 测试断言兼容
  kind?: FragmentKind; // 稳定 ASCII key，避免编码/显示差异导致的抽取问题
  text: string;
}
 ...
 * 按修复闭环方向：抽具体“风险/反驳/建议”等语义片段，而不是盲目截断。
 * 使用稳定 kind 内部表示 + 中文 label 展示，降低 mojibake / 环境编码风险。
 */
export function extractArtifactFragments(artifact: Pick<Artifact, "title" | "summary" | "content">, maxLength = 140): ArtifactFragment[] {
...
/**
 * buildStructuredReport — V5 主输出物升级（证据级 9-section 报告）
 *
 * 按用户本次 Findings 要求，将 report.write 的 content 从“半模拟聚合字符串”
 * 升级为固定 schema 的可信推演报告：
 *
 *   结论
 *   支撑证据
 *   反证/挑战
 ...
 *   provenance / upstream refs
 *
 * 这个 helper 纯函数，接收 state + 本次 report 的 inputArtifactIds（executor 友好）。
 * 内部复用 extractArtifactFragments + hasStale 判定 + upstream 遍历。
 * 未来真实 executor（LLM/MCP）可以产出等价结构或更丰富版本，但 surface 合同不变。
 *
 * 同时被 simulateCapabilityExecution（默认路径）和页面 send/reentry 消费。
 */
...
export function buildStructuredReport(input: StructuredReportInput): { title: string; summary: string; content: string } {
  ...
  // Build rich fragments from all upstreams (risk/synth/evidence etc)
  ...
  // Focused risk / rebuttal / suggestion fragments (for 反证/挑战 + 风险 sections)
  ...
  // Exactly the 9 sections requested (结论 / 支撑证据 / 反证/挑战 / 风险 / 分歧 / 收敛决策 / 未解缺口 / 下一步工程化分支 / provenance)
